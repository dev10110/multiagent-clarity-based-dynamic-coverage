...
function (wd::WindData)(t)
    return wd(wd.X, wd.Y, t)
end
...
function load_axes(fn; starts = [1, 1, 1], counts = [-1, -1, -1])
    ...
    Y =
        Float64.(
            (ncread(fn, "Y", start = [starts[2]], count = [counts[2]]) |> reverse) / 1000
        )
    T =
        round.(
            Float64.(
                ncread(fn, "time"; start = [starts[3]], count = [counts[3]]) .- T_OFFSET
            )
        )
...
function get_filename(month, day)
    ...
    fn =
        data_root *
        @sprintf("/WN_L2_HHD_v8.0_UTM_CALMETWIND_JBT_UTC_2023-%02d-%02d.nc", month, day)
    return fn
end
...
function load_data(fn; starts = [1, 1, 1], counts = [-1, -1, -1])
    ...
    W_dir =
        ncread(fn, "wind_dir_HHM"; start = [1; starts], count = [1; counts]) .* W_dir_scale
...
@recipe function f(d::WD, t::F; plottype = :wx) where {F<:AbstractFloat,WD<:WindData}

    t_datetime = to_datetime(t)
    # get the actual data
    wdx, wdy = d(d.X, d.Y, t)

    @assert plottype in (:wx, :wy, :mag, :dir)
